...
// ✅ JOB CARD - Apply Now ঠিক করা
function createJobCard(job) {
    const vacancies = job.vacancies ? job.vacancies.toLocaleString("en-IN") : "See Notification";
    // applyUrl না থাকলে officialUrl ব্যবহার করবে
    const applyLink = job.applyUrl && job.applyUrl !== "#" ? job.applyUrl : job.officialUrl;
    ...
    return `
        <article class="job-card">
            ...
            <div class="job-actions">
                <a href="job-detail.html?id=${encodeURIComponent(job.id)}" class="btn btn-outline">View Details</a>
                <a href="${escapeHTML(applyLink)}" target="_blank" rel="noopener noreferrer" class="btn btn-primary">Apply Now</a>
            </div>
        </article>
    `;
}
...
// ✅ JOB DETAIL PAGE - Apply Now ঠিক করা
function setupJobDetailPage() {
    ...
    // ✅ applyUrl চেক করা - যদি না থাকে তাহলে officialUrl ব্যবহার করবে
    const applyLink = job.applyUrl && job.applyUrl !== "#" ? job.applyUrl : job.officialUrl;
